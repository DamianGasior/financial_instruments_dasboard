...
class Underlying_data_frame:
    def __init__(
        ...
        response_from_alpha,
        key_paremeter,
        underlying_request: Underlying_request_details,
    ):
        self.stock_symbol = (
            underlying_request.symbol
        )  # class instance from Underlying_request_details
        self.response_from_alpha = response_from_alpha
        self.close = self.stock_symbol
        self.transform(key_paremeter).set_date_as_index().column_rename(
            **{"1. open": "open"},
            **{"2. high": "high"},
            **{"3. low": "low"},
            **{"4. close": self.close},
